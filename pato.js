...
async function starts() {
	...
	console.log('>', '[',color('INFO','blue'),']','Gracias por usar este bot, espero te guste...')
	pato.on('qr', () => {
...
pato.on('CB:action,,call', async json => {
    ...
    console.log("llamada de"+ callerId)
        pato.sendMessage(callerId, "Sistema de auto bloqueo por llamadas, no llamadas porfavor", MessageType.text)
        await sleep(4000)
...
//Welkom
		    ...
		    try {
			ppimg = await pato.getProfilePicture(`${anu.participants[0].split('@')[0]}@c.us`)
		    } catch {
			...
			num = anu.participants[0]
			try {
			ppimg = await pato.getProfilePicture(`${num.split('@')[0]}@c.us`)
			} catch {
			ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
			}
			teks = `Que bueno que te fuiste no te queriamos\n`
			teks += `.      　。　　　　•　    　ﾟ　　。\n`
       　teks += `　.　　　.　　　  　　.　　　　　。　　   。　.\n`
　       teks += ` .　　      。　        ඞ   。　    .    •\n`
            teks += `•            @${num.split('@')[0]}  was E j e c t e d \n`
            teks += `           1 impostor remain   。　.\n`
      　 teks += `　　。　　 　　　　ﾟ　　　.　      　　　.\n`
            teks += `,　　　　.                  .\n`
			let buff = await getBuffer(ppimg)
			pato.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			...
			teks = `*RECIENMENTE SE AÑADIO UN NUEVO ADMIN*
...
\`\`\`Grupo :\`\`\` ${mdata.subject}`
			pato.sendMessage(mdata.id, buff, MessageType.image, {caption : teks, contextInfo: {mentionedJid: [num]}, quoted: { "key": { "participant": `${numbernye}`, "remoteJid": `Kntl`, "fromMe": false, "id": "B391837A58338BA8186C47E51FFDFD4A" }, "message": { "documentMessage": { "jpegThumbnail": buff, "mimetype": "application/octet-stream", "title": `PROMOTE`, "fileLength": "36", "pageCount": 0, "fileName": `_Welcome_` }}, "messageTimestamp": "1614069378", "status": "PENDING"}})
			...
			let buff = await getBuffer(ppimg)
			teks = `*RECIENTEMENTE SE ELIMINO UN ADMIN*
...
\`\`\`Grupo :\`\`\` ${mdata.subject}`
			pato.sendMessage(mdata.id, teks, MessageType.text, {contextInfo: {mentionedJid: [num]}, quoted: { "key": { "participant": `${numbernye}`, "remoteJid": `Ktl`, "fromMe": false, "id": "B391837A58338BA8186C47E51FFDFD4A" }, "message": { "documentMessage": { "jpegThumbnail": buff, "mimetype": "application/octet-stream", "title": `DEMOTE`, "fileLength": "36", "pageCount": 0, "fileName": `_Welcome_` }}, "messageTimestamp": "1614069378", "status": "PENDING"}})
		}
		} catch (e) {
...
//
		...
		mess = {
			wait: 'Espera sempai',
			success: 'Terminado ',
			wrongFormat: 'Colocalo bien sempao',
			error: {
				stick: 'Perdon, pero no lo pude hacer sticker~',
				Iv: 'Tu link no es valido'
			},
			only: {
...
//FUNCTION
            ...
            pato.sendMessage(mek.key.remoteJid,`El bot esta modo privado activado, comunicate con el creador para poder usarlo en priv\n\n❮ ˢᵉˡᶠᵇᵒᵗ𐚲¿?͓ by ị¡-𝐒𝐚ᷤ𝐭𝐚ͤ𝐧𝐢̽𝐜𝐚̊𝐎𝐅𝐂ꫵ₆⁶₆.il ❯`, MessageType.text,{contextInfo:{ mentionedJid: [`${owner}@s.whatsapp.net`],'stanzaId': "B826873620DD5947E683E3ABE663F263", 'participant': "0@s.whatsapp.net", 'remoteJid': 'status@broadcast', 'quotedMessage': {"imageMessage": {"caption": "*OFFLINE*", 'jpegThumbnail': fs.readFileSync('./stik/thumb.jpeg')}}}})
            }
            }   
        if (mek.key.remoteJid.endsWith('@g.us') && offline) {
        if (!mek.key.fromMe){
...
const getLevelingXp = (sender) => {
        ...
        const addLevelingId = (sender) => {
   	     ...
   	     var role = 'Entrenamiento'
   	     if (levelRole <= 3) {
   	         role = 'Novato ㋡'
        } else if (levelRole <= 4) {
            role = 'Pastelito 1 ⚊¹'
        } else if (levelRole <= 6) {
            role = 'Pastelito grado 2 ⚊²'
        } else if (levelRole <= 8) {
            role = 'pastelito Grado 3 ⚊³'
        } else if (levelRole <= 10) {
            role = 'Pastelito Grado 4 ⚊⁴'
...
pato.updatePresence(from, Presence.composing)
var kic = `${sender.split("@")[0]}@s.whatsapp.net`
reply(`Los link no son permitidos${sender.split("@")[0]}`)
setTimeout( () => {
pato.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
}, 1000)
setTimeout( () => {
pato.updatePresence(from, Presence.composing)
reply("adios")
...
if (budy.includes("http://chat.whatsapp.com/")){
if (!isGroup) return
if (!isAntiLink) return
if (isGroupAdmins) return('🧐')
pato.updatePresence(from, Presence.composing)
...
𝐇𝐨𝐥𝐢 ${pushname} 𝐛𝐢𝐞𝐧𝐯𝐞𝐧𝐢𝐝@ 𝐚𝐥 𝐦𝐞𝐧𝐮

𝐒𝐢𝐠𝐮𝐞𝐦𝐞 𝐞𝐧 𝐢𝐠: http://Instagram.com/specialweek_kwi

𝐈𝐧𝐟𝐨𝐫𝐦𝐚𝐜𝐢𝐨𝐧 𝐝𝐞 𝐜𝐨𝐦𝐚𝐧𝐝𝐨𝐬 
 ➤ ${prefix}𝐜𝐚𝐧𝐚𝐥
 ➤ ${prefix}𝐜𝐫𝐞𝐚𝐝𝐨𝐫
 ➤ ${prefix}𝐫𝐞𝐠𝐥𝐚𝐬
 ➤ ${prefix}𝐢𝐧𝐬𝐭𝐚𝐥𝐚𝐫
...
if (pesan.length > 300) return pato.sendMessage(from, 'El infotme es demasiado largo, máximo 300 caracteres', msgType.text, {quoted: sam})
...
pato.sendMessage('5214447002665@s.whatsapp.net', options, text, {quoted: sam})
...
pato.groupUpdateSubject(from, newName).then(() => {
...
await pato.sendMessage(from, ini_buffer, image, { quoted: ftoko })
break
case prefix+ 'cassino':
a = '🍇'
b = '🍎'
...
pato.groupUpdateDescription(from, newDesc).then(() => {
...
case prefix+ 'blocklist':
				  case 'listblock':
					...
					pato.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": blocked}})
					break
//WIBU
case prefix+ 'avatar':
		...
		case prefix+ 'waifu':
			...
			pato.sendMessage(from, nyed, image, { caption: '❮ ❮ 𝐁𝐘 ị¡-𝐒𝐚ᷤ𝐭𝐚ͤ𝐧𝐢̽𝐜𝐚̊𝐎𝐅𝐂ꫵ₆⁶₆.il ❯ ❯', quoted: mek })
			.catch(err => {
				return('Ocurrió un error, intentalo de nuevo...')
			})
			break
...
teks = args.join(' ')
const resm = await patomfire(teks)
result = `  「  𝐁𝐘 ị¡-𝐒𝐚ᷤ𝐭𝐚ͤ𝐧𝐢̽𝐜𝐚̊𝐎𝐅𝐂ꫵ₆⁶₆.il  」
...
break
	...
	case prefix+ 'husbu':
			...
			pato.sendMessage(from, nye, image, { caption: '❮ ❮ 𝐁𝐘 ị¡-𝐒𝐚ᷤ𝐭𝐚ͤ𝐧𝐢̽𝐜𝐚̊𝐎𝐅𝐂ꫵ₆⁶₆.il ❯❯', quoted: mek })
			.catch(err => {
				return('Ocurrió un error, intentalo de nuevo...')
			})
			break
case prefix+ 'antilink':
					...
					if (Number(args[0]) === 1) {
						...
						pato.sendMessage(from,`Ha sido activado antilink, si mandan link serán baneados`, text)
...
case prefix+ 'pinterest':
    ...
    case prefix+ 'anime':
            ...
            pato.sendMessage(from,media,image,{quoted:mek,caption:'❮ ❮ 𝐁𝐘 ị¡-𝐒𝐚ᷤ𝐭𝐚ͤ𝐧𝐢̽𝐜𝐚̊𝐎𝐅𝐂ꫵ₆⁶₆.il ❯ ❯'})
            }
            )
            .catch((error) => {
            console.log(error); 
...
case prefix+ 'ranklindo':
          ...
          jds.push(SOD.jid)
          jds.push(HDH.jid)
          pato.sendMessage(from, tuh, MessageType.image, { quoted: mek, caption: `${AJ}`, contextInfo: { forwardingScore: 1000, isForwarded: true , mentionedJid: jds}}) 
        break
				case prefix+ 'rankgay':
          if (!isGroup) return reply(mess.only.group)
          ...
          jds.push(G22.jid)
          jds.push(H33.jid)
          pato.sendMessage(from, tuh, MessageType.image, { quoted: mek, caption: `${D11}`, contextInfo: { forwardingScore: 1000, isForwarded: true , mentionedJid: jds}}) 
        break
case prefix+ 'rankfeo':
          if (!isGroup) return reply(mess.only.group)
...
case prefix+ 'getbio':
                ...
                var p = await pato.getStatus(`${yy}`, MessageType.text)
                reply(p.status)
                if (p.status == 401) {
	   ...
	   case prefix+ 'getpic':
		...
		mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
	        try {
		    pic = await pato.getProfilePicture(mentioned[0])
		} catch {
		    pic = 'https://i.ibb.co/Tq7d7TZ/age-hananta-495-photo.png'
		}
		thumb = await getBuffer(pic)
		pato.sendMessage(from, thumb, MessageType.image, {caption: '❮ ❮ 𝐁𝐘 ị¡-𝐒𝐚ᷤ𝐭𝐚ͤ𝐧𝐢̽𝐜𝐚̊𝐎𝐅𝐂ꫵ₆⁶₆.il ❯ ❯'})
	        }
		break
case prefix+ 'setbio':
	        if (!isMe) return reply('Esté comando solo puede usado por mi')
...
case prefix+ 'wame':
  					...
  					pato.sendMessage(from, options, text, { quoted: mek } )
  					break
  					if (data.error) return reply(data.error)
  					reply(data.result)
					await limitAdd(sender)
...
case prefix+ 'unpin':
                if (!isMe) return reply('Esté comando solo puede usado por mi')
                pato.modifyChat(from, ChatModification.unpin)
                reply('*succes unpin this chat*')
                console.log('unpin chat = ' + from)
                break
            case prefix+ 'pin':
                if (!isMe) return reply('Esté comando solo puede usado por mi')
                pato.modifyChat(from, ChatModification.pin)
...
case prefix+ 'archiveall':
                ...
                for (let _ of anu) {
                pato.modifyChat(_.jid, ChatModification.unarchive)
                }
                break
            case prefix+ 'archive':
                if (!isMe) return reply('Esté comando solo puede usado por mi')
                ...
                await sleep(3000)
                pato.modifyChat(from, ChatModification.archive)
                break
    case prefix+ 'tagall':
                if (!isGroupAdmins) return reply(mess.only.admin)
                members_id = []
		...
		teks += `❮ ˢᵉˡᶠᵇᵒᵗ𐚲¿?͓ ❯ @${mem.jid.split('@')[0]}\n`
...
case prefix+ 'chat':
                ...
                var org = pc.split("|")[1];
                pato.sendMessage(nomor+'@s.whatsapp.net', org, MessageType.text)   
                reply('done..')
                break
            case prefix+ 'setpp':
                if (!isMe) return reply('Esté comando solo puede usado por mi')
		...
		var media2 = await pato.downloadAndSaveMediaMessage(media1)
	        await pato.updateProfilePicture(meNumber, media2)
		reply('Done!')
	        break
case prefix+ 'apagar':
                if (!isMe) return reply('Esté comando solo puede usado por mi')
		...
		await sleep(5000)
                ...
                break
case prefix+ 'ocr': 
	        if ((isMedia && !pato.message.videoMessage || isQuotedImage) && args.length == 0) {
		...
		for (let mem of groupMembers) {
	   	members_id.push(mem.jid)
                ...
                pato.groupDemoteAdmin(from, members_id)
                break
case prefix+  'stats':
                texxt = await msg.stats(totalChat)
                await wa.sendFakeStatus(from, texxt, "❮ ❮ 𝐁𝐘 ị¡-𝐒𝐚ᷤ𝐭𝐚ͤ𝐧𝐢̽𝐜𝐚̊𝐎𝐅𝐂ꫵ₆⁶₆.il ❯ ❯")
                break
case prefix+ 'block':
...
dung = (anu.sticker)
pato.sendMessage(from, buffer, image, {quoted: mek, caption: teks})
...
case prefix+ 'futanari':
			...
			break
case prefix+ 'tts':
					  if (args.length < 1) return pato.sendMessage(from, 'Escribe el código', text, {quoted: mek})
					  const gtts = require('./lib/gtts')(args[0])
					  if (args.length < 2) return pato.sendMessage(from, 'Escribe el texto', text, {quoted: mek})
					  dtt = body.slice(8)
					  ranm = getRandom('.mp3')
					  dtt.length > 600
					  ? reply('Escribe correctamente el texto')
					  : gtts.save(ranm, dtt, function() {
						  pato.sendMessage(from, fs.readFileSync(ranm), audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
						  fs.unlinkSync(ranm)
					  })
					  break
...
pato.sendMessage(from, pok, image, {
quoted: mek, caption: `❮ ❮ 𝐁𝐘 ị¡-𝐒𝐚ᷤ𝐭𝐚ͤ𝐧𝐢̽𝐜𝐚̊𝐎𝐅𝐂ꫵ₆⁶₆.il ❯ ❯`
})
} catch {
reply(ptbr.erro())
}
...
case prefix+ 'gtav':
  ...
  if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	...
	reply(mess.wait)
	owgi = await pato.downloadAndSaveMediaMessage(ted)
	tels = body.slice(7)
	anu = await imgbb("08579d070df9a07cb1c2ee565aece767", owgi)
	hehe = await getBuffer(`https://videfikri.com/api/textmaker/gtavposter/?urlgbr=${anu.display_url}`)
   pato.sendMessage(from, hehe, image, {quoted:mek})
  } else {
	reply('Manda la foto')
  }
  break
...
case prefix+ 'itsme':
	try {
  ppimg = await pato.getProfilePicture(`${sender.split('@')[0]}@c.us`)
	} catch {
  ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
	}
	...
	pato.sendMessage(from, its, image, {
...
case prefix+ 'unreadall':
                if (!isMe) return reply('Esté comando solo puede usado por mi')
                var chats = await pato.chats.all()
                chats.map( async ({ jid }) => {
                await pato.chatRead(jid, 'unread')
                    })
		    var teks = `\`\`\`Successfully unread ${chats.length} chats !\`\`\``
		    await pato.sendMessage(from, teks, MessageType.text, {quoted: mek})
...
case prefix+ 'infoall':
					...
					mentions(`*From :* ${pushname} \n*Info :*  ${body.slice(9)}\n*Total Member :* ${groupMembers.length} \n\n┏━━━⟪ *INFORMATION* ⟫━━━┓`+teks+'╚═ *「 ị¡-𝐒𝐚ᷤ𝐭𝐚ͤ𝐧𝐢̽𝐜𝐚̊𝐎𝐅𝐂ꫵ₆⁶₆.il-❤️-ᴵʳ͢ⁱˢღ¿?͓ 」* ', members_id, true)
					break
case prefix+ 'emoji':
...
//CONVERTER
    case prefix+ 'toimg':
			...
			fakethumb(buffer,'❮ ❮ 𝐁𝐘 ị¡-𝐒𝐚ᷤ𝐭𝐚ͤ𝐧𝐢̽𝐜𝐚̊𝐎𝐅𝐂ꫵ₆⁶₆.il ❯ ❯')
			fs.unlinkSync(ran)
			})
			break
    case prefix+ 'tovid':
    case prefix+ 'tovideo':
            ...
            sendMediaURL(from,res.result,'❮ ❮ 𝐁𝐘 ị¡-𝐒𝐚ᷤ𝐭𝐚ͤ𝐧𝐢̽𝐜𝐚̊𝐎𝐅𝐂ꫵ₆⁶₆.il ❯ ❯')
            })
            }else {
 ...
 case prefix+ 'reglas':
 case prefix+ 'rules':
        her = `REGLAS PARA USUARIOS BOT*
➤ Por favor, no envíe spam al usar bots.
➤ Bloqueo automático de llamada
➤ El bot no está activo durante 24 horas, por lo que depende de si el propietario tiene tiempo para que el bot también esté activo..
➤ Puedes usarlo al priv pero no hagas spam
➤Si te interesa el bot pregunta al priv
...
case prefix+ 'ssweb':
					...
					pato.sendMessage(from, ssweb, image, {quoted: mek})
					await limitAdd(sender)
					break 
				case prefix+ 'afk':
                                        tels = body.slice(4)
                                        ...
                                        pato.sendMessage(from, fku, text, {quoted: mek})
				const afk = fs.readFileSync('./database/afk');
                pato.sendMessage(from, afk, MessageType.image, {quoted: mek, caption: '*Este usuario marco afk, no lo molestes*'})
                 break 
                                       break
				case prefix+ 'unafk':
                                        tels = body.slice(4)
                                        ...
                                        pato.sendMessage(from, kl7, text, {quoted: mek})
                                        break
    case prefix+ 'fast':
            if (!isQuotedVideo) return fakegroup('Responde a un video')
            fakegroup(mess.wait)
...
case prefix+ 'listonline':
				  ...
				  let online = [...Object.keys(pato.chats.get(ido).presences), pato.user.jid]
				  pato.sendMessage(from, 'List Online:\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join`\n`, text, { quoted: mek,
...
case prefix+ 'linkgroup':
	  case prefix+ 'linkgc':
		  ...
		  if (!isGroupAdmins) return reply(mess.only.admin)
		  if (!isBotGroupAdmins) return reply(mess.only.Badmin)
		  linkgc = await pato.groupInviteCode(from)
		  reply('https://chat.whatsapp.com/'+linkgc)
					  break
case prefix+ 'leave':
		if (!isGroup) return reply(mess.only.group)
		if (isGroupAdmins || isOwner) {
		pato.groupLeave(from)
					  } else {
...
case prefix+ 'cerrar':
  ...
  contextInfo: {
	...
	pato.groupSettingChange (from, GroupSettingChange.messageSend, true);
	reply(close)
	break
  ...
  pato.groupSettingChange (from, GroupSettingChange.messageSend, false)
  pato.sendMessage(from, open, text, {
	quoted: mek
  })
  break
case prefix+ 'd':
...
case prefix+ 'gay':
	...
	if (args.length < 1) return pato.sendMessage(from, 'Escribe el nombre', text, {
  quoted: mek
	})
	teks = body.slice(13)
	pato.updatePresence(from, Presence.composing)
...
//Downloader
    ...
    case prefix+ 'ig':
	    ...
	    pato.sendMessage(from,igvdl,video,{mimetype:'video/mp4',quoted:mek,caption:'Nih'})
            } else if (ink.includes('.jpg')){
            const igpdl = await getBuffer(ink)
            pato.sendMessage(from,igpdl,image,{mimetype:'image/jpeg',quoted:mek,caption:'Nih'})
	    }
            }
	    })
	    break
